...
namespace ScpMessages
{
	...
	{
		// Modified to support a new Mutatable Tuple class (TwoTupleSO) and a List holding these Tuples (TwoTupleSOList)
		...
		*/
		...
		public static string ReplaceAfterToken(this string source, char token, TwoTupleSOList valuePairs)
		{
			if (valuePairs == null)
			{
			...
			do
			{
				...
				if (auxChar == token)
				{
					...
					// Try to find a tuple
					foreach (TwoTupleSO kvp in valuePairs.Tuples)
					{
						int j, k;
